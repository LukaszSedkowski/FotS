...
public class ChessBoard : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        scoreManager.InitializePlayers(playerIds);
        ...
        if (PlayerPrefs.HasKey("Player_0_Username"))
        {
            Debug.Log("Tryb turniejowy - ładowanie danych graczy...");
            LoadTournamentPlayers();
        }
        else
        {
            Debug.Log("Tryb gościa - standardowe ustawienia...");
            SpawnAllPieces();
        }
        ...
        if (chessPieces[0, 0] == null)
        {
            Debug.LogError("chessPieces[0, 0] is null!");
        }
        else
        {
    ...
    private void LoadTournamentPlayers()
    {
        int playerCount = PlayerPrefs.GetInt("TeamCount", 2);
        chessPieces = new ChessPieces[Tile_Count_X, Tile_Count_Y];  // Inicjalizacja tablicy pionków

        int pieceID = 0;  // Rozpocznij od indeksu 0
        int team = 0;

        for (int playerIndex = 0; playerIndex < playerCount; playerIndex++)
        {
            string savedPieces = PlayerPrefs.GetString($"Player_{playerIndex}_Pieces", "");
            string username = PlayerPrefs.GetString($"Player_{playerIndex}_Username", "Unknown");
            string token = PlayerPrefs.GetString($"Player_{playerIndex}_Token", "NoToken");

            if (string.IsNullOrEmpty(savedPieces))
            {
                Debug.LogWarning($"Brak zapisanych pionków dla drużyny {playerIndex + 1} ({username})");
                continue;
            }

            string[] pieceNames = savedPieces.Split(',');
            ...
            foreach (string pieceName in pieceNames)
            {
                if (!string.IsNullOrEmpty(pieceName))
                {
                    try
                    {
                        ChessPieceType type = (ChessPieceType)System.Enum.Parse(typeof(ChessPieceType), pieceName);
                        ChessPieces piece = SpawnSinglePiece(type, team, pieceID + 1, username, token);

                        if (piece != null)
                        {
                            // Znajdź pierwsze wolne miejsce w rzędzie
                            int x = pieceID % Tile_Count_X;  // Umieszczaj na szerokości
                            int y = pieceID / Tile_Count_X;  // Kolejny rząd

                            // Upewnij się, że nie przekroczysz granic planszy
                            if (x < Tile_Count_X && y < Tile_Count_Y)
                            {
                                chessPieces[x, y] = piece;
                                pieceID++;  // Inkrementuj po każdym dodaniu pionka
                                Debug.Log($"Stworzono pionek {pieceName} dla gracza {username} na ({x}, {y}). (Team: {team})");
                            }
                            else
                            {
                                Debug.LogError($"Brak miejsca na planszy dla pionka {pieceName} (Team: {team}).");
                            }
                        }
                        else
                        {
                            Debug.LogError($"Nie udało się stworzyć pionka {pieceName} dla drużyny {team}.");
                        }
                    }
                    catch (System.Exception e)
                    {
                        Debug.LogError($"Błąd parsowania pionka: {pieceName}. Wyjątek: {e.Message}");
                    }
                }
            }
            team++;
        ...
        // Sprawdzenie, czy jakikolwiek pionek został dodany
        if (pieceID == 0)
        {
            Debug.LogError("Żaden pionek nie został dodany! Sprawdź konfigurację pionków w PlayerPrefs.");
        }
    }
    ...
    private void GameOver()
    {
        ...
        ScoreManager.Instance.SendStatsForAllPlayers();
        ...
        /*for (int i = 0; i < PlayerPrefs.GetInt("TeamCount", 2); i++)
        {
            ...
            if (userId != -1)
            {
                bool won = (i == currentTeam); // Aktualna drużyna to wygrana
                scoreManager.EndGame(userId, won);
            }
        }
    ...
    private void AttackEnemyPiece(int targetX, int targetY)
    {
        ...
        if (targetPiece.team != currentlyDragging.team)
        {
            ...
            if (distance <= currentlyDragging.attackRange) // Zakładam, że masz pole attackRange w ChessPieces
            {
                ...
                {
                    int attackerUserId = PlayerPrefs.GetInt($"Player_{currentlyDragging.team}_UserId", -1);
                    int victimUserId = PlayerPrefs.GetInt($"Player_{targetPiece.team}_UserId", -1);

                    if (attackerUserId != -1)
                    {
                        scoreManager.RecordKill(attackerUserId);
                    }

                    if (victimUserId != -1)
                    {
                        scoreManager.RecordDeath(victimUserId);
                    }

                    Debug.Log($"Atak: {attackerUserId} zniszczył jednostkę gracza {victimUserId}");
    ...
    private ChessPieces SpawnSinglePiece(ChessPieceType type, int team, int id, string playerName = "", string token = "")
    {
        int prefabIndex = (int)type - 1;

        if (prefabIndex < 0 || prefabIndex >= prefabs.Length || prefabs[prefabIndex] == null)
        {
            Debug.LogError($"Błąd: Brak prefabrykatu dla {type} (Index: {prefabIndex})");
            return null;
        }

        ChessPieces cp = Instantiate(prefabs[prefabIndex], transform).GetComponent<ChessPieces>();

        if (cp == null)
        {
            Debug.LogError($"Błąd: Nie udało się utworzyć pionka {type} dla drużyny {team}.");
            return null;
        }

        cp.Init(type, team, id, playerName, token);
    ...
    private void PositionAllPieces()
    {
        for (int x = 0; x < Tile_Count_X; x++)
        {
            for (int y = 0; y < Tile_Count_Y; y++)
            {
                ...
                {
                    Debug.LogWarning($"Brak pionka na pozycji {x}, {y}. Pomijam pozycjonowanie.");
                }
            }
        }
    }
